...
Простой запуск:
    python sat_lg_test_v3.py --formulas 50 --vars 20 --clauses 80 --k 3 --max-steps 80 --lookaheads 2,3

Средний запуск:
    python sat_lg_test_v3.py --formulas 100 --vars 30 --clauses 120 --k 3 --max-steps 120 --lookaheads 2,3

Файлы:
    sat_lg_v3_summary_by_mode.csv
    sat_lg_v3_summary_by_formula.csv
    sat_lg_v3_logs.jsonl
"""
...
def main():
    ...
    parser.add_argument("--out-prefix", type=str, default="sat_lg_v3")

    args = parser.parse_args()

    lookaheads = [int(x.strip()) for x in args.lookaheads.split(",") if x.strip()]

    print("=== CONFIG SAT LG V3 ===")
    ...
    for i in range(args.formulas):
        ...
        greedy_l = search_sat(
            formula, args.vars, formula_seed,
            mode="greedy_L",
            score_kind="L",
            lookahead_depth=1,
            max_steps=args.max_steps,
        )
        all_results.append(greedy_l)
        if len(selected_logs) < args.log_limit:
            selected_logs.append(greedy_l)
        ...
        all_results.append(greedy_lg)
        if len(selected_logs) < args.log_limit:
            selected_logs.append(greedy_lg)
        ...
        for depth in lookaheads:
            ...
            all_results.append(lg_res)
            if len(selected_logs) < args.log_limit:
                selected_logs.append(lg_res)
