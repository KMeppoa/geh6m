...
Простой запуск:
    python sat_l_test_v2_fixed.py --formulas 100 --vars 30 --clauses 120 --k 3 --max-steps 100 --lookaheads 1,2,3,5

Более большой запуск:
    python sat_l_test_v2_fixed.py --formulas 500 --vars 50 --clauses 210 --k 3 --max-steps 200 --lookaheads 1,2,3,5

Файлы результата:
    sat_l_v2_fixed_summary_by_mode.csv
    sat_l_v2_fixed_summary_by_formula.csv
    sat_l_v2_fixed_logs.jsonl
"""
...
def complete_assignment(partial_assignment, n_vars):
    """
    DPLL может найти частичное решение:
    все клаузы уже satisfied, но часть переменных не важна.
    Для строгой проверки дополняем их любым значением.
    """
...
def dpll_control(formula, n_vars, formula_seed, max_nodes):
    ...
    full_result = complete_assignment(result, n_vars)
    success, verify_logs = verify_assignment(formula, full_result, n_vars)
    logs.extend(verify_logs)
    logs.append(
        f"FINISH dpll_control: SAT={success}, nodes={node_counter[0]}, "
        f"partial_assigned={len(result)}, full_assigned={len(full_result)}"
    )
    return RunResult(formula_seed, "dpll_control", success, node_counter[0], full_result, logs)
...
def main():
    ...
    parser.add_argument("--out-prefix", type=str, default="sat_l_v2_fixed")

    args = parser.parse_args()
    lookaheads = [int(x.strip()) for x in args.lookaheads.split(",") if x.strip()]

    print("=== CONFIG SAT L V2 FIXED ===")
